...
namespace FIMSpace.Generating
{
    public partial class OStamperMultiSet
    {
        ...
        public class MultiStamperSetParameters : PrefabReference
        {
                ...
                if (TargetSet != null && TargetSet.Prefabs != null)
                    if (TargetSet.Prefabs.Count > 0)
                        if (TargetSet.Prefabs[0] != null)
                            Prefab = TargetSet.Prefabs[0].Prefab;
            }

            internal int GetRandomLimitCount()
            {
            ...
            internal int GetRandomLimitCount(int prefabId, OStamperMultiSet set)
            {
                ...
                //PrefabSettings pr;
                GameObject prefab = TargetSet.Prefabs[prefabId].Prefab;

                if (prefab == null) return 0;

                for (int i = 0; i < set.PerPrefabSettings.Count; i++)
                {
                    if (set.PerPrefabSettings[i].Prefab == prefab)
                    {
            ...
            protected override void DrawGUIWithoutPrefab(int previewSize = 72, string predicate = "", Action removeCallback = null)
            {
                if (TargetSet != null)
            ...
            protected override void DrawGUIWithPrefab(Color color, int previewSize = 72, string predicate = "", Action clickCallback = null, Action removeCallback = null, bool drawThumbnail = true)
            {
                if (TargetSet == null)
                {
                    EditorGUILayout.HelpBox("Assign Stamper Set!", MessageType.Error);
