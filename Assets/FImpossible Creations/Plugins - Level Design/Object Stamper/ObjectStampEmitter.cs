...
namespace FIMSpace.Generating
{
    ...
    public class ObjectStampEmitter : ObjectStampEmitterBase, IGenerating
    {
        ...
        internal void _EditorEmitPreview()
        {
            ClearPreviews();
            _editorPreview = SpawnEmitPrefab(PrefabsSet);
        }

        internal void _EditorEmitAndDetach()
        {
            ClearPreviews();
                ...
                if (spawningInfo.ChoosedPrefab != null)
                {
                    ...
                    if (prr != null)
                        if (prr.GetMesh() != null)
                        {
                            ...
                            Gizmos.DrawMesh(prr.GetMesh(), Gizmos.matrix.inverse.MultiplyPoint(transform.position));
                        }
                }
                ...
                if (h.transform)
                {
                    ...
                    if (spawnPrefabRef != null)
                        if (spawnPrefabRef.GetMesh() != null)
                        {
                            ...
                            Gizmos.DrawMesh(spawnPrefabRef.GetMesh(), drawPos);
                        }
                }
            }
        }
