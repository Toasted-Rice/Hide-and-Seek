...
namespace FIMSpace.Generating
{
    ...
    public class GridPainter : PGGGeneratorBase
    {
        ...
        public override void GenerateObjects()
        {
            ...
            if (Generated == null) Generated = new List<FieldGenerationInfo>();
            ...
            for (int i = 0; i < CellsInstructions.Count; i++)
            {
                ...
                if (instr.CustomDefinition == null) continue;

                SpawnInstruction guide = new SpawnInstruction();
                Vector3 dir = instr.rot * Vector3.forward;
                ...
                if (instr.CustomDefinition.InstructionType != InstructionDefinition.EInstruction.None)
                    ...
                    if (instr.Id < FieldPreset.CellsInstructions.Count)
                    guide.definition = FieldPreset.CellsInstructions[instr.Id];
            ...
            for (int i = 0; i < ignoredForGenerating.Count; i++) { FieldPreset.Ignores.Add(ignoredForGenerating[i]); }
            ...
            if (Injections != null) if (Injections.Count > 0) FieldPreset.SetTemporaryInjections(Injections);
            ...
            try
            {
                Generated.Add(IGeneration.GenerateFieldObjects(FieldPreset, grid, transform, true, guides, null, true));
                if (Injections != null) if (Injections.Count > 0) FieldPreset.ClearTemporaryInjections();
                FieldVariablesRestore();
            }
            catch (System.Exception exc)
            {
                UnityEngine.Debug.LogError("[PGG] Error when generating with GridPainter: " + exc);
                if (Injections != null) if (Injections.Count > 0) FieldPreset.ClearTemporaryInjections();
                FieldVariablesRestore();
            }
            ...
            for (int i = 0; i < ignoredForGenerating.Count; i++) FieldPreset.Ignores.Remove(ignoredForGenerating[i]);
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            for (int i = 0; i < CellsInstructions.Count; i++)
            {
                Color nCol = Color.HSVToRGB((float)CellsInstructions[i].Id * 0.1f % 1f, 0.7f, 0.8f);
                nCol.a = Transprent ? 0.185f : 1f;
                Gizmos.color = nCol;
                Vector3 pos = FieldPreset.TransformCellPosition((Vector3)CellsInstructions[i].pos);
                                    ...
                                    if (!is2D)
                                    {
                                        for (int x = -rad; x < rad; x++)
                                            for (int ry = 0; ry < _Editor_RadiusY; ry++)
                                                ...
                                                {
                                                    if (x == 0 && ry == 0 && z == 0) continue;
                                                    Gizmos.DrawWireCube(gridPos + new Vector3(cSize.x * x, cSize.y * ry + cSize.y * 0.5f, cSize.z * z), new Vector3(cSize.x, cSize.y, cSize.z * 0.2f));
                                                }
                                    }
                                    else
                                    {
