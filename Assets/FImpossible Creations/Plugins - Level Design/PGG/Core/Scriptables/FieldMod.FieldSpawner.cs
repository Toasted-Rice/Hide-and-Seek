...
namespace FIMSpace.Generating
{
    ...
    public partial class FieldSpawner
    {
        ...
        public FieldSpawner(int stampPrefabID, FieldModification.EModificationMode mode, FieldModification parent)
        {
            ...
            {
                if (SObjectAvailable()) Name = parent.GetPrefabRef(StampPrefabID).Prefab.name; else Name = "NULL!";
            }
        }
        ...
        /// </summary>
        public SpawnData RunSpawnerOnCell(FieldModification mod, FieldSetup preset, FieldCell cell, FGenGraph<FieldCell, FGenPoint> grid, Vector3 desiredDirection, FieldModification childMod, bool dontUseGlobalRules = false, bool ignoreRestrictions = false)
        {
            _EditorRefreshSpawner();
            ...
            if (MultipleToSpawn)
            {
                ...
                if (selected.Length > 0)
                    spawnData = SpawnData.GenerateSpawn(this, childMod == null ? mod : childMod, cell, selected[FGenerators.GetRandom(0, selected.Length)]);
            }


            if (spawnData == null) spawnData = SpawnData.GenerateSpawn(this, childMod == null ? mod : childMod, cell, StampPrefabID);
            ...
            if (_RulesToCheck.Count == 0) allRequirementsMet = true; // No rules then just inject-spawn
                ...
                for (int i = 0; i < _RulesToCheck.Count; i++) // Check all rules logic requirements
                {
                    ...
                    var result = rule.RunSpawnMod(preset, mod, cell, ref spawnData, grid);
            ...
            if (cell.HaveInstructions())
            {
                ...
                {
                    var instr = cell.GetInstructions()[i];
                    if (instr.IsPostSpawn == false) continue;
