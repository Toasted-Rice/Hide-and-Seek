...
namespace FIMSpace.Generating
{
    public abstract partial class SpawnRuleBase
    {
        ...
        public virtual void NodeHeader()
        {
            ...
            if (string.IsNullOrEmpty( _in_head) )
            {
                _in_head = "  " + TitleName().Replace("SR_", "") + "  ";
            }

            if (GUILayout.Button(new GUIContent(foldout + _in_head + foldout, tip), FGUI_Resources.HeaderStyle))
            {
        ...
        public static bool DrawMultiCellSelector(List<Vector3Int> cells, FieldSpawner spawner, bool yellowIfZero = false)
        {
            ...
            if (GUILayout.Button(new GUIContent(CheckCellsSelectorUtils.Tex_Selector, "Open window for advanced neightbour cells selection - node rule will be checked on neightbour cells"), FGUI_Resources.ButtonStyle, GUILayout.Width(19), GUILayout.Height(17))) { CheckCellsSelectorWindow.Init(cells, spawner); return true; }
        ...
        public static bool DrawMultiCellSelector(CheckCellsSelectorSetup setup, FieldSpawner spawner, bool yellowIfZero = false)
        {
            ...
            if (GUILayout.Button(new GUIContent(CheckCellsSelectorUtils.Tex_Selector, "Open window for advanced neightbour cells selection - node rule will be checked on neightbour cells"), FGUI_Resources.ButtonStyle, GUILayout.Width(19), GUILayout.Height(17))) { CheckCellsSelectorWindow.Init(null, spawner, setup); return true; }
